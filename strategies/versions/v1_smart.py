...
class AdvancedTradingAlgorithm:
    ...
    def execute_strategy(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(data)):
            ...
            if combined_signal != 0:
                signal_strength = min(
                    max(primary_signal["strength"], secondary_signal["strength"]),
                    1.0
                )
            else:
                signal_strength = 0.0
            ...
            if combined_signal != 0:
                primary_reason = self.get_signal_reason(data, i, combined_signal)
                secondary_reason = f"Regime: {regime}, Strength: {signal_strength:.2f}"
            else:
                primary_reason = primary_signal.get("reason", "No primary signal")
                secondary_reason = secondary_signal.get("reason", "No secondary signal")
    ...
    def generate_signal(self, data: pd.DataFrame, idx: int) -> int:
        """
        Smart separated signal generator with regime filter.
        ...
        # Map regime to trend
        if regime in ["STRONG_UPTREND", "TRENDING"]:
            trend = "UP"
        ...
        mean_rev_buy = z_score < -2.0 and rsi < 35 and bb_position < 0.2
        mean_rev_sell = z_score > 2.0 and rsi > 65 and bb_position > 0.8

        # Momentum criteria
        rsi_prev = data["RSI"].iloc[idx - 1] if idx > 0 else rsi
        ema12 = current.get("EMA_12", current['close'])
        ema26 = current.get("EMA_26", current['close'])
        macd = current.get("MACD", 0)
        macd_signal = current.get("MACD_signal", 0)

        mom_buy = rsi > 55 and rsi > rsi_prev and ema12 > ema26 and macd > macd_signal
        mom_sell = rsi < 45 and rsi < rsi_prev and ema12 < ema26 and macd < macd_signal

        # Use regime to choose strategy
        if trend == "UP":
            # Prefer momentum long
            if mom_buy:
                return 1
            else:
                return 0
        elif trend == "DOWN":
            # Prefer momentum short
            if mom_sell:
                return -1
            else:
                return 0
